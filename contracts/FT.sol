...
contract FT {
    ...
    // 4. reward slaves
    function rewardFriend(address _slaveAD, uint reward) public payable returns( uint){
        uint _slaveID = addrs[_slaveAD];
        User storage _slave = users[_slaveID];
        ...
        _user.balance -=reward;
        uint extra = msg.value - reward;
        if (extra > 0) {
            msg.sender.transfer(extra);
        }
    ...
    function GetUser(uint id)  view public returns(address ad, uint balance, uint value,uint masterID,
    ...
    // get id
    function getId(address ad)  view public returns(uint id){
